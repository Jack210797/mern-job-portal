...
const ViewApplications = () => {
  ...
  // Fuction to fetch company job applicantions data
  const fetchCompanyJobApplications = async () => {
    try {
      ...
      if (data.success) {
        setApplicants(data.applicants ? data.applicants.reverse() : [])
      } else {
  ...
  return applicants ? (
    applicants.length === 0 ? (
      ...
      <div className="container mx-auto p-4">
        <div>
          <table className="w-full max-w-4xl bg-white border border-gray-200 max-sm:text-xs">
            <thead>
